...
APP_COL = BUTTON_COLORS["exrm_tk"]
font_col = "snow"

# Undo用（一世代のみ保持）
undo_backup_dir = None
undo_base_dir = None
undo_deleted_dirs = []

def start_undo_session(base_dir):
    global undo_backup_dir, undo_base_dir, undo_deleted_dirs
    if undo_backup_dir and os.path.isdir(undo_backup_dir):
        shutil.rmtree(undo_backup_dir, ignore_errors=True)
    undo_backup_dir = tempfile.mkdtemp(prefix="exrm_undo_")
    undo_base_dir = os.path.abspath(base_dir)
    undo_deleted_dirs = []


def backup_remove_file(path):
    rel_path = os.path.relpath(path, undo_base_dir)
    backup_path = os.path.join(undo_backup_dir, rel_path)
...
def undo_click(event):
    ...
    if not undo_backup_dir or not os.path.isdir(undo_backup_dir) or not undo_base_dir:
        msg.insert(tk.END, "Undo可能な削除履歴がありません。\n", "info")
        msg.config(state="disabled")
        return

    # 削除された空DIRを親階層から復元
    for rel_dir in sorted(undo_deleted_dirs, key=lambda p: p.count(os.sep)):
        os.makedirs(os.path.join(undo_base_dir, rel_dir), exist_ok=True)

    restored = 0
    conflicts = 0
    for root_dir, _, files in os.walk(undo_backup_dir):
        for name in files:
            backup_path = os.path.join(root_dir, name)
            rel_path = os.path.relpath(backup_path, undo_backup_dir)
            restore_path = os.path.join(undo_base_dir, rel_path)
            os.makedirs(os.path.dirname(restore_path), exist_ok=True)
            if os.path.exists(restore_path):
                msg.insert(tk.END, "conflict  " + restore_path + "\n", "war1")
                conflicts += 1
                continue
            shutil.move(backup_path, restore_path)
            msg.insert(tk.END, "restore   " + restore_path + "\n", "info")
            restored += 1
    ...
    if conflicts == 0:
        ...
        undo_base_dir = None
        undo_deleted_dirs = []
        msg.insert(tk.END, f"\nUndo完了 : {restored} file(s) restored.\n", "info")
    else:
        msg.insert(tk.END, f"\nUndo未完了 : {conflicts}件の競合があります。\n", "war2")
...
def help_click(event):
    ...
    msg.insert(tk.END, "  Undo                 : 直前のExRemoveで削除した内容を復元します（一世代）。\n" , "info")
    msg.config(state="disabled")
            ...
            if os.path.isdir(del_nm):                    # 処理対象がDIR
                if bln0.get():                            # DIR再帰処理フラグ有
                    ...
                    try:
                        ...
                        undo_deleted_dirs.append(os.path.relpath(del_nm, undo_base_dir))
...
if __name__ == '__main__':
    ...
    try:
        root.iconbitmap(resource_path("exrm_tk.ico"))
...
##############
    ...
    grad_draw(canvas1, APP_COL["select"], grad_size)
...
#- ExRemove Button ------------------------------------------------
    ...
    grad_draw(canvas1, COM_COL["exec"], grad_size)
...
#- Undo Button ----------------------------------------------------
    ...
    grad_draw(canvas2, COM_COL["undo"], grad_size)
    canvas2.create_text(font_x, font_y, text="Undo", fill=font_col, font=(font_knd, font_siz))
    canvas2.bind("<Button-1>", undo_click)                        # クリックイベントをバインド
#- Clear Button ---------------------------------------------------
    ...
    grad_draw(canvas3, COM_COL["clear"], grad_size)
    canvas3.create_text(font_x, font_y, text="Clear", fill=font_col, font=(font_knd, font_siz))
    canvas3.bind("<Button-1>", clear_click)                        # クリックイベントをバインド
#- Help Button ----------------------------------------------------
    canvas4 = tk.Canvas(frame4, width=canvas_wid, height=canvas_hgt, highlightthickness=0)
    canvas4.create_rectangle(0, 0, grad_wid, canvas_hgt, fill=line_col, outline=line_col)
    canvas4.pack(side="left", padx=6)
    grad_draw(canvas4, COM_COL["help"], grad_size)
    canvas4.create_text(font_x, font_y, text="Help", fill=font_col, font=(font_knd, font_siz))
    canvas4.bind("<Button-1>", help_click)                        # クリックイベントをバインド
#- Dummy Label ----------------------------------------------------
...
#- Exit Button ----------------------------------------------------
    ...
    grad_draw(canvas5, COM_COL["exit"], grad_size)
