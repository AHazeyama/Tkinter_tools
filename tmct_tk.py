...
def settings_path():
    """初期設定保存ファイルのパス"""
    return Path.home() / ".tmct_tk_settings.json"


def solid_color(color_spec):
    """tk_colors.py のグラデーション定義から濃色側を単色として使用"""
    return "#" + "".join(color_spec[3:6])
...
class DigitalClockTimer(tk.Tk):
    def __init__(self):
        ...
        self.default_second = 20
        self.default_target_sets = 5
        self.load_defaults()
    ...
    def create_widgets(self):
        ...
        self.create_button(
            ...
            0, solid_color(APP_COL["start"]), "#ffffff", button_font
        )
        self.create_button(
            button_frame, "Stop", self.stop_timer,
            1, solid_color(APP_COL["stop"]), "#ffffff", button_font
        )
        self.create_button(
            button_frame, "Clear", self.clear_timer,
            2, solid_color(COM_COL["clear"]), "#ffffff", button_font
        )
        self.create_button(
            button_frame, "Default", self.save_defaults,
            3, solid_color(APP_COL["default"]), "#ffffff", button_font
        )
        self.create_button(
            button_frame, "Exit", self.destroy,
            4, solid_color(COM_COL["exit"]), "#ffffff", button_font
        )

        self.shortcut_label = tk.Label(
            self,
            text="F5: Start   F6: Stop   F7: Clear   F8: Default   Esc: Exit",
        ...
        self.hour_var = tk.StringVar(value=str(self.default_hour))
        self.minute_var = tk.StringVar(value=str(self.default_minute))
        self.second_var = tk.StringVar(value=str(self.default_second))
        ...
        self.target_sets_var = tk.StringVar(value=str(self.default_target_sets))
    ...
    def load_defaults(self):
        ...
        try:
            ...
            self.default_hour = int(data.get("hour", 0))
            self.default_minute = int(data.get("minute", 0))
            self.default_second = int(data.get("second", 20))
            self.default_target_sets = int(data.get("target_sets", 5))
        except (OSError, ValueError, TypeError, json.JSONDecodeError) as e:
            print(f"Default setting load error: {e}")


    def save_defaults(self):
        """現在のタイマー時間と目標セット数を初期値として保存"""
        try:
            ...
            messagebox.showerror(
                "Error",
                "時・分・秒・目標セット数には数値を入力してください。"
            )
            return

        if hour < 0 or minute < 0 or second < 0:
            messagebox.showerror("Error", "時・分・秒は0以上にしてください。")
            return

        if hour * 3600 + minute * 60 + second <= 0:
            messagebox.showerror("Error", "タイマーは1秒以上にしてください。")
            return
        ...
        try:
            ...
            messagebox.showerror("Error", f"初期設定を保存できませんでした。\n{e}")
            return

        self.default_hour = hour
        self.default_minute = minute
        self.default_second = second
        self.default_target_sets = target_sets

        messagebox.showinfo(
            "Default",
            f"初期値を保存しました。\n"
            f"Timer : {hour:02}:{minute:02}:{second:02}\n"
            f"Sets  : {target_sets}"
        )
    ...
    def bind_keys(self):
        ...
        self.bind("<F8>", lambda e: self.save_defaults())
    ...
    def clear_timer(self):
        ...
        self.hour_var.set(str(self.default_hour))
        self.minute_var.set(str(self.default_minute))
        self.second_var.set(str(self.default_second))
        self.target_sets_var.set(str(self.default_target_sets))
