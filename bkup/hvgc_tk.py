...
def check_click(event):
	...
	text3.delete("1.0", tk.END)
	try:
		if sub_path_entry.get():
			if sel_no == 1:
				hash_key = hashlib.md5(file_path.encode()).hexdigest() 
			elif sel_no == 2:
				hash_key = hashlib.sha1(file_path.encode()).hexdigest()
			elif sel_no == 3:
				hash_key = hashlib.sha3(file_path.encode()).hexdigest()
			elif sel_no == 4:
				hash_key = hashlib.sha256(file_path.encode()).hexdigest()
			elif sel_no == 5:
				hash_key = hashlib.sha512(file_path.encode()).hexdigest()
			elif sel_no == 6:
				hash_key = hashlib.blake2b(file_path.encode()).hexdigest()
	
			gen_key_entry.config(state="normal")
			gen_key_entry.delete(0, END)
			gen_key_entry.insert(0, hash_key)
			gen_key_entry.icursor(0)					# カーソルを先頭に移動
			gen_key_entry.config(state="readonly")
			if exp_key_entry.get():
				if exp_key_entry.get() == gen_key_entry.get():
					text3.insert(tk.END, "Mach !!\n", "judge_OK")
					text3.insert(tk.END, "  生成したHashKey\"Generation Hash\"と期待値\"Hash Expectation\"が一致しました。", "msg")
				else:
					text3.insert(tk.END, "Discrepancy !!\n", "judge_NG")
					text3.insert(tk.END, "  生成したHashKey\"Generation Hash\"と期待値\"Hash Expectation\"が一致しませんでした。", "error")
			else:
				text3.insert(tk.END, "\"Generation Hash\"が生成されました。", "info")
		else:
			text3.insert(tk.END, "[Select] ボタンで生成対象\"Check jsubject\"を選択して下さい。", "info")
	except tk.TclError:
		pass  # クリップボードが空などでエラーになる場合も無視
	text3.config(state="disabled")
...
def clear_click(event):
	...
	gen_key_entry.config(state="readonly")
	text3.config(state="normal")
	text3.insert(tk.END, "[Select] ボタンで生成対象を選択し、 \"Hash Algorithm\"を選択してから、[Check] ボタンをクリックして下さい。\n\"Generated Hash\"にHash Keyが生成されます。\n", "info")
...
if __name__ == '__main__':
	root = Tk()
	root.title('Hash generation and expected value check tool [hashgc]')
...
##############
	...
	grad_draw(canvas1_3, grad_col, grad_size)
...
#	canvas2.grid(row=0,column=0, sticky=W)
	...
	canvas2.create_rectangle(10, 2, 150, 20, fill="lightblue", outline="")
	canvas2.create_text(80, 10, text="Hash Algorithm", fill="black")
	selected = tk.IntVar()
	selected.set(4)  # 初期選択（例）

	labels = ["HD5", "SHA-1", "SHA-3", "SHA-256", "SHA-512", "BLAKE2"]

	for i, label in enumerate(labels):
		rb = tk.Radiobutton(canvas2, text=label, variable=selected, value=i+1, bg="lightblue", width=7, anchor="w", highlightthickness=0)
		col = i % 3           # 0, 1, 2
		row = i // 3          # 0, 1
		x = 80 + col * 120    # X位置（3列）
		y = 38 + row * 25     # Y位置（2行）
